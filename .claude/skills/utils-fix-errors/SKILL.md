...
allowed-tools: Bash(bun :*), Bash(tsc :*), Read, Task, Grep
---
...
2. **RUN DIAGNOSTICS**:
   ...
   - Run `bun run check-types` or `tsc --noEmit`
...
7. **FORMAT CODE**: Apply Biome (included in check command)
   - Run `bun run check` (auto-fixes formatting)

...
if [[ -z "$FEATURE_ID" ]] || [[ -z "$STEP_NUMBER" ]] || [[ -z "$STEP_NAME" ]] || [[ -z "$STATUS" ]]; then
    echo "Usage: $0 <feature_id> <step_number> <step_name> <status>"
    echo "Example: $0 F016 01 context complete"
    exit 1
fi

# Find project root
PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
CONTEXT_FILE="${PROJECT_ROOT}/.nomos/output/${FEATURE_ID}/00-context.md"

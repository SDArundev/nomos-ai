...
### 5. Pre-Implementation Check (CRITICAL)

**Check if the feature is ALREADY IMPLEMENTED in the codebase.**

Compare the feature's acceptance criteria against findings:
...
```bash
# Mark as pre-implemented and skip to learning
bash {skill_dir}/scripts/feature-state.sh preverify {feature_id}
```

Then:
1. Document what exists and where
2. Skip to step-09-learn.md (extract patterns from existing implementation)
3. Do NOT proceed to planning/execution

**IF NOT all criteria met:**
→ Continue to step 6 (Infer Acceptance Criteria)
